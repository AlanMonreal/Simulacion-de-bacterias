...
temp = tk.DoubleVar()
temp.set(36.5)
temp_scale = tk.Scale(root, from_=40.0, to=33, variable=temp, length=260, resolution=0.1)
temp_label = tk.Label(root, text="Temperatura", font=("Helvetica", 11))
temp_scale.place(x=680, y=40)
temp_label.place(x=645, y=15)

acidity = tk.DoubleVar()
acidity.set(7.4)
acidity_scale = tk.Scale(root, from_=8, to=7.2, variable=acidity, length=260, resolution=0.001)
acidity_label = tk.Label(root, text="Acidez", font=("Helvetica", 11))
acidity_scale.place(x=740, y=40)
acidity_label.place(x=740, y=15)

nutrient = tk.IntVar()
nutrient.set(4)
nutrient_scale = tk.Scale(root, from_=10, to=4, variable=nutrient, length=260)
nutrient_label = tk.Label(root, text="Nutriente", font=("Helvetica", 11))
nutrient_scale.place(x=800, y=40)
nutrient_label.place(x=795, y=15)

humidity = tk.IntVar()
humidity.set(80)
humidity_scale = tk.Scale(root, from_=100, to=65, variable=humidity, length=260)
humidity_label = tk.Label(root, text="Humedad", font=("Helvetica", 11))
humidity_scale.place(x=860, y=40)
humidity_label.place(x=860, y=15)

temp_label = tk.Label(root, text="Temperatura: ", font=("Helvetica", 12))
acidity_label = tk.Label(root, text="Acidez: ", font=("Helvetica", 12))
nutrient_label = tk.Label(root, text="Nutriente: ", font=("Helvetica", 12))
humidity_label = tk.Label(root, text="Humedad: ", font=("Helvetica", 12))
temp_live_label = tk.Label(root, textvariable=temp, font=("Helvetica", 12))
acidity_live_label = tk.Label(root, textvariable=acidity, font=("Helvetica", 12))
nutrient_live_label = tk.Label(root, textvariable=nutrient, font=("Helvetica", 12))
humidity_live_label = tk.Label(root, textvariable=humidity, font=("Helvetica", 12))
...
bacteria_label = tk.Label(root, text="Bacteria", font=("Helvetica", 20))
bacteria_1 = tk.Button(root, text="M. tuberculosis", command=lambda:setSelectedBacteria(0))
# bacteria_2 = tk.Button(root, text="H. influenzae", command=lambda: setSelectedBacteria(1))
# bacteria_3 = tk.Button(root, text="M. pneumoniae", command=lambda: setSelectedBacteria(2))
# bacteria_4 = tk.Button(root, text="S. pyogenes", command=lambda:setSelectedBacteria(3))
# bacteria_5 = tk.Button(root, text="E. coli", command=lambda: setSelectedBacteria(4))
# bacteria_6 = tk.Button(root, text="P. mirabilis", command=lambda: setSelectedBacteria(5))
...
sel_bact_label = tk.Label(root, text="Bacteria seleccionada: ", font=("Helvetica", 12))
sel_bact_label.place(x=525, y=300)


start_button = tk.Button(root, text="Empezar", command=start)
pause_button = tk.Button(root, text="Pausar", command=pause)
restart_button = tk.Button(root, text="Reiniciar", command=restart)
start_button.place(x=780, y=350)
pause_button.place(x=755, y=390)
restart_button.place(x=810, y=390)

# save_image_button = tk.Button(root, text="Guardar imagen", command=save_image)
# save_image_button.place(x=525, y=460)
open_image_button = tk.Button(root, text="Ver imagen", command=open_image)
